...
class TelegramBot():
    ...
    def __init__(self):
        ...
        def gmail_handler(message):
            ...
            match args[0]:
                ...
                case 'jobs':
                    ...
                    job_items = []
                    for items in jobs:
                        job_items.extend(read_linkedin_job_alert(items))

                    self.bot.reply_to(
                        message, f'You have <strong>{len(job_items)}</strong> unread jobs, sir.\nHere are all the jobs that you didn\'t read yet.\n', parse_mode='HTML')
        ...
        def read_linkedin_job_alert(job):
            message = self.gmail_service.users().messages().get(
                userId='me', id=job['id']).execute()
